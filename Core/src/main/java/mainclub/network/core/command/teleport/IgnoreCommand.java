...
public class IgnoreCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        if(!sender.hasPermission("core.command.tpignore")) {
        ...
        if (manager.tpaIgnore().containsKey(((Player)sender).getUniqueId()) && manager.tpaIgnore().get(((Player)sender).getUniqueId()).contains(target.getUniqueId())) {
            manager.tpaIgnore().get(((Player)sender).getUniqueId()).remove(target.getUniqueId());
            sender.sendMessage("Ya no ignoras las solicitudes de teletransportaciones de  "+target.getName()+".");
            return false;
        }
        manager.tpaIgnore().get(((Player)sender).getUniqueId()).add(target.getUniqueId());
        sender.sendMessage("Ahora ignoras las solicitudes de teletransportaciones de "+target.getName()+".");
        return false;
    }
}
